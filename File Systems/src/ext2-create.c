...
void write_block_group_descriptor_table(int fd) {
	...
	/* These are intentionally incorrectly set as 0, you should set them
	   correctly and delete this comment */
	block_group_descriptor.bg_block_bitmap = BLOCK_BITMAP_BLOCKNO; // what block is your block given in (defines)
	block_group_descriptor.bg_inode_bitmap = INODE_BITMAP_BLOCKNO;
	block_group_descriptor.bg_inode_table = INODE_TABLE_BLOCKNO; // inode start
...
void write_block_bitmap(int fd) {
	/* This is all you */
	off_t offset = lseek(fd, BLOCK_OFFSET(BLOCK_BITMAP_BLOCKNO), SEEK_SET);
	if (offset == -1){
		errno_exit("lseek");
	}

	
	u8* mapOfBits = calloc (NUM_BLOCKS, sizeof(u8)); 

	u32 i = 0;
	while (i < NUM_BLOCKS*8 ){ 
		off_t byteOffset = i / 8;
		off_t bit_off = i % 8;
		
		
		if (i < 23 || i >= 1023){
			mapOfBits[byteOffset] |= (1 << bit_off); 
		}
		i++;
	}

	if (write(fd, mapOfBits, NUM_BLOCKS) != NUM_BLOCKS){
		errno_exit("write");
	}
...
void write_inode_bitmap(int fd) {
	/* This is all you */
	off_t offset = lseek(fd, BLOCK_OFFSET(INODE_BITMAP_BLOCKNO), SEEK_SET);
	if (offset == -1){
		errno_exit("lseek");
	}
	...
	while (i < NUM_BLOCKS*8){
		...
		if (i < 13 || i >= 128){
			mapOfBits[byteOffset] |= (1 << bitOffset);
		}
		i++;
	}
...
void write_inode_table(int fd) {
	...
	hello_world_inode.i_size = BLOCK_SIZE;
...
void write_root_dir_block(int fd) {
	/* This is all you */
	off_t offset =  lseek(fd, BLOCK_OFFSET(ROOT_DIR_BLOCKNO), SEEK_SET);
	if (offset == -1) {
		errno_exit("lseek");
	}
	...
	remainingBytes -= hello_world_file.rec_len;

	struct ext2_dir_entry hello_entry = {0};
	dir_entry_set(hello_entry, HELLO_INO, "hello");
	dir_entry_write(hello_entry, fd);
	
	remainingBytes -= hello_entry.rec_len;

	dir_entry_write_fill(remainingBytes, fd);
}
...
void write_hello_world_file_block(int fd) {
	...
	off_t offset =  lseek(fd, BLOCK_OFFSET(HELLO_WORLD_FILE_BLOCKNO), SEEK_SET);
	if (offset == -1) {
		errno_exit("lseek");
	}
	...
	struct ext2_dir_entry curr_entry = {0};
	dir_entry_set(curr_entry, HELLO_WORLD_INO, ".");
	dir_entry_write(curr_entry, fd);

	remainingBytes -= curr_entry.rec_len;

	struct ext2_dir_entry parent_entry = {0};
	dir_entry_set(parent_entry, HELLO_WORLD_INO, "..");
	dir_entry_write(parent_entry, fd);
