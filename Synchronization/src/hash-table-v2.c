...
struct hash_table_v2 *hash_table_v2_create()
{
	...
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		...
		pthread_mutex_init(&entry->mutex_attr, NULL);
	}
	return hash_table;
}
                             ...
                             uint32_t value)
{
	...
	pthread_mutex_lock(&hash_table_entry->mutex_attr);
	...
	pthread_mutex_unlock(&hash_table_entry->mutex_attr);
}
...
void hash_table_v2_destroy(struct hash_table_v2 *hash_table)
{
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		...
		while (!SLIST_EMPTY(list_head)) {
			...
			pthread_mutex_destroy(&entry->mutex_attr);
			free(list_entry);
		}
	}
	free(hash_table);
